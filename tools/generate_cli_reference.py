...
def _render_cli_index(
    ...
    lines.extend(
        [
            ...
            "--------------------------",
            "",
            "Most plotting, mapping, and visualization commands can resolve their standard input tables and figure paths from the active config, so ``--input-table``/``--input`` and ``--figure-output``/``--output`` are optional for the usual tutorial/workflow outputs. Registered table defaults may be CSV or Parquet depending on the configured output key; commands that say they accept CSV or parquet read either suffix through the package table helpers. Use ``svtk plot metrics list``, ``svtk plot spatial list``, ``svtk map spatial list``, ``svtk visualize qc list``, ``svtk visualize context list``, or ``svtk visualize waveforms list`` to see which commands use ``config:<key>`` defaults and which intentionally require explicit input tables, shown as ``required:<role>`` entries. Add ``--resolve-paths --config PATH`` to any of those list commands when you want to see the concrete configured paths that will be used. A ``required:<role>`` entry means that command works on a caller-supplied table outside the standard output registry, so pass ``--input-table``/``--input`` or a named table flag for that invocation.",
            "",
...
def _registered_input_default_meaning(spec: PlotCommand) -> str:
    ...
    return f"This advanced figure requires an explicit table. Pass ``--input-table`` or ``--input``{suffix}."
