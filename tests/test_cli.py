...
from spatial_vtk.cli import _write_table
...
def test_cli_write_table_uses_shared_csv_parquet_writer(tmp_path, capsys):
    """CLI table outputs should use the same writer contract as package helpers."""

    output = tmp_path / "rows"
    table = pd.DataFrame({"station": ["STA001"], "value": [1.5]})

    written = _write_table(table, output)

    captured = capsys.readouterr()
    assert written == tmp_path / "rows.csv"
    assert captured.out.strip() == str(written)
...
def test_generated_cli_reference_includes_config_backed_examples():
    ...
    assert "Plot residual or score distributions by passband." in plot_text
    assert "Plot score distributions by passband." not in plot_text
    assert (
