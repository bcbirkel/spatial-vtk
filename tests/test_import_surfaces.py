...
def test_large_run_metric_helpers_keep_matplotlib_lazy():
    """Large-run metric aggregation/status helpers should import without plotting deps."""

    root = pathlib.Path(__file__).resolve().parents[1]
    source_path = root / "src" / "spatial_vtk" / "metrics" / "plot" / "large_run.py"
...
def test_visualize_api_docs_use_public_entry_points():
    ...
    assert "``readiness`` / ``status_reason`` / ``message``" in text
    assert "``ready`` / ``readiness`` / ``status_reason`` / ``message``" in text
...
def test_large_run_csv_readers_use_stable_dtype_inference():
    ...
    repo_root = pathlib.Path(__file__).resolve().parents[1]
    snippets = {
        "src/spatial_vtk/qc/build/inventory.py": "return read_table(path)",
...
def test_slurm_and_qc_workflow_helpers_defer_config_imports():
    """Slurm and QC workflow helpers should not require config dependencies to import."""

    repo_root = pathlib.Path(__file__).resolve().parents[1]
    checks = {
        "src/spatial_vtk/metrics/workflow/slurm.py": {
            "spatial_vtk.config.compute",
            "spatial_vtk.config.runtime",
        },
        "src/spatial_vtk/qc/build/slurm.py": {
            "spatial_vtk.config.compute",
            "spatial_vtk.config.metrics",
            "spatial_vtk.config.outputs",
            "spatial_vtk.config.runtime",
        ...
        "src/spatial_vtk/qc/build/workflow.py": {
            "spatial_vtk.config.metrics",
            "spatial_vtk.config.outputs",
            "spatial_vtk.config.runtime",
            "spatial_vtk.io.tables",
